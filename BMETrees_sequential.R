...
BMETrees_sequential <- function(data_x, subject_id, time, number_of_MCMC){
  df <- data_x
  subject <- subject_id
  time <- time
  ncol_df <- ncol(df)
  ...
  for (h in 1:number_of_MCMC){
    ...
    # Update thetas
    BMETrees_1_sigest <- BMETrees_1$sigest
    BMETrees_1_tau <- BMETrees_1$tau
    BMETrees_1_sigma <- BMETrees_1$sigma
    BMETrees_1_fit <- BMETrees_1$fit
    BMETrees_1_delta <- BMETrees_1$delta
    ...
    # Update thetas
    df_impute_2_sigest <- BMETrees_2$sigest
    df_impute_2_tau <- BMETrees_2$tau
    df_impute_2_sigma <- BMETrees_2$sigma
    df_impute_2_fit <- BMETrees_2$fit
    df_impute_2_delta <- BMETrees_2$delta
    
    # Impute missing
    df_reorder[,"x_1"] <- BMETrees_1$yhat.test.mean
    df_reorder[,"x_2"] <- BMETrees_2$yhat.test.mean
    ...
    x_1_h <- as.numeric(predict(BMETrees_1, newdata = df_impute_1_updated, group.by = df_impute_1_updated$subject_id))
    x_1_last_h <- BMETrees_1$yhat.test.mean
    x_1_acccept <- rnorm(x_1_h, BMETrees_1_sigma)/rnorm(x_1_last_h, BMETrees_1_sigma)
