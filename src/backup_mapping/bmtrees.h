...
get_inverse_wishart_matrix2 = function(X, Y, Z, subject_id, subject_to_B, binary = F){
  ...
  if(!binary){
    suppressMessages(lmm <- lmer(Y ~ X + (0 + Z|subject_id)))
    coe = ranef(lmm)[[1]]
    coe = (coe[names(subject_to_B),])
  }else{
    suppressMessages(lmm <- glmer(Y ~ X + (0 + Z|subject_id), family = binomial(link = "logit")))
    coe = ranef(lmm)[[1]]
    coe = (coe[names(subject_to_B),])
  ...
  #print(subject_to_B)
  #print(coe)
  print(as.matrix(bdiag(VarCorr(lmm))))
...
get_inverse_wishart_matrix = function(Y, Z, subject_id, subject_to_B, binary = F){
  if(!binary){
    suppressMessages(lmm <- lmer(Y ~ 0 + (0 + Z|subject_id)))
    coe = ranef(lmm)[[1]]
    coe = (coe[names(subject_to_B),])
  }else{
    suppressMessages(lmm <- glmer(Y ~ 0 + (0 + Z|subject_id), family = binomial(link = "logit")))
    coe = ranef(lmm)[[1]]
    coe = (coe[names(subject_to_B),])
  }
  return(list(coe = as.matrix(coe), sigma = sigma(lmm), covariance = as.matrix(bdiag(VarCorr(lmm)))))
...
# get_inverse_wishart_matrix = function(X, Y, Z, subject_id){
#   print(Z)
#   lmm = lmer(Y ~ X + (0 + Z|subject_id))
#   coe = ranef(lmm)[[1]]
#   print(coe)
#   return(cov(coe))
# }
...
update_tree = function(X, y){
  ...
  tree = gbart(X, y, verbose = 0, ndpost=1000, nskip = 100)
  #print(y)
  return(tree)
...
class bmtrees{
public:
  bmtrees(NumericVector Y, NumericMatrix X, Nullable<NumericMatrix> Z, CharacterVector subject_id, IntegerVector row_id, bool binary = false, bool CDP_residual = false, bool CDP_re = false, double tol=1e-40, int ntrees = 50, int pattern = 0, double pi_CDP = 0.99) {     // Constructor
    
    G = Rcpp::Environment::global_env();
    Function get_inverse_wishart_matrix2 = G["get_inverse_wishart_matrix2"];
    ...
    if(CDP_re){
      ...
      B_tau = DP(List::create(Named("p") = d, Named("cov") = as<NumericMatrix>(inverse_wishart_matrix)), M_re, sqrt(n_subject), n_subject, false);
    ...
    tree = new bart_model(this->X, this->Y - re - tau_samples, 100L, false, false, false, 50);
    if(CDP_residual){
  ...
  void update_all(bool verbose = false){
    ...
    //return List::create(subject_to_B)；


    if(verbose)
      Rcout << "update B" << std::endl;
    NumericVector test_a = Y - tau_samples - tree_pre;
    Rcout <<  test_a.length() << std::endl;
    ...
    if(pattern == 0){
      ...
      if(CDP_residual){
        ...
        for(int i = 0 ; i < n ; ++i){
          ...
          //   e[i] = sample(values, 1, false, pi)[0];
          // }
          NumericVector loc = sample(values, pattern, true, pi);
          e[i] = 0;
          for(int k = 0; k < pattern; ++k){
            e[i] += R::rnorm(loc[k], sigma);
          }
          e[i] = e[i] / pattern;
