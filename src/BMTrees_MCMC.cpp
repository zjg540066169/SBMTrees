...
// [[Rcpp::export]]
List BMTrees_MCMC(LogicalVector obs_ind, long nburn, long npost, NumericVector Y, NumericMatrix X, Nullable<NumericMatrix> Z, CharacterVector subject_id, bool binary = false, bool verbose = true, Nullable<long> seed = R_NilValue, double tol = 1e-40, bool CDP_residual = false, bool CDP_re = false, int pattern = 0, double pi_CDP = 0.99, long  ntrees = 50){
  // if(CDP_residual){
  ...
  //return List::create();
  bmtrees model = bmtrees(clone(Y_obs), clone(X_obs), clone(Z_obs), subject_id_obs, row_id_obs, binary, CDP_residual, CDP_re, tol,  ntrees ,pattern, pi_CDP);
  //return List::create();
  //
  //
