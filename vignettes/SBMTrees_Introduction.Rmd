...
We then run the prediction model `BMTrees`, with 300 burn-in iterations and 400 posterior samples. The number of burn-in and posterior iterations should be increase to 3000 and 4000, respectively. Here we only use the small numbers to simply debug.
```{r prediction}
...
We then run the sequential imputation with `BMTrees`, with 300 burn-in iterations, 400 posterior iterations, and sample one posterior sample for every 200 posterior iterations, ensuring 20 multiply-imputed sets are generated. The number of burn-in and posterior iterations should be increase to 3000 and 4000, respectively. Here we only use the small numbers to simply debug.
```{r imputation}
